...
The =treemacs-all-the-icons= theme overrides the icon for folders named =docs=,
=doc=, or =documentation= with a single "file-text" glyph that does *not*
include the expand chevron, which throws off vertical alignment in the tree.
Re-register those folder names with the standard chevron + folder icon so they
line up like every other directory.
